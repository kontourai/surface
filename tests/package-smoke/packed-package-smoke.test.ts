    ...
    const importCheck = await execFileAsync(
      ...
      [
        ...
        [
          ...
          "import { TrustBundleBuilder, buildAnswerCardProjection, buildTrustReport, validateTrustBundle, explainClaim, projectReviewedExtractionEvidence, buildReviewedExtractionSourceState, evaluateReviewedGroundingPolicy } from '@kontourai/surface';",
          ...
          "if (typeof projectReviewedExtractionEvidence !== 'function') throw new Error('reviewed evidence projection missing');",
          "if (typeof buildReviewedExtractionSourceState !== 'function') throw new Error('reviewed source-state builder missing');",
          "if (typeof evaluateReviewedGroundingPolicy !== 'function') throw new Error('reviewed grounding policy missing');",
          `const reviewedFixture = JSON.parse(await readFile(${JSON.stringify(path.join(root, "tests", "fixtures", "reviewed-extraction-evidence.v1.json"))}, 'utf8'));`,
          "const projected = projectReviewedExtractionEvidence(reviewedFixture);",
          "const sha = (value) => ({ algorithm: 'sha256', value: value.repeat(64).slice(0, 64) });",
          "const observation = { version: 'surface.reviewed-source-observation/v1', owner: { authority: '@kontourai/fieldwork', observationRef: 'fieldwork:packed-consumer' }, expected: { snapshotRef: 'snapshot:fixture-v1', sourceId: 'forage:source-1', resourceRef: 'https://example.test/directory', capturedAt: '2026-07-20T00:00:00.000Z', envelopeDigest: sha('a'), contentDigest: sha('b') }, observed: { snapshotRef: 'forage:capture-304', sourceId: 'forage:source-1', resourceRef: 'https://example.test/directory', capturedAt: '2026-07-20T00:00:00.000Z', envelopeDigest: sha('c'), contentDigest: sha('b') } };",
