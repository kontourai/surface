...
export interface ThreadAnswerRef { authority: "@kontourai/thread"; schemaVersion: "1.2.0"; kind: "assistant-message"; threadId: string; messageId: string; }
...
export interface BasisOwnerDescriptor { authority: string; }
export type BasisOwnerRef = BasisContributionRef | SurfaceAnswerAssessmentRef;
export type OwnerRead<T, TRef extends BasisOwnerRef = BasisOwnerRef> =
  | { owner: BasisOwnerDescriptor; state: "available"; observedAt: string; value: T }
  | { owner: BasisOwnerDescriptor; state: "observed-empty"; observedAt: string; value: readonly [] }
  | { owner: BasisOwnerDescriptor; state: "not-captured"; observedAt: string }
  | { owner: BasisOwnerDescriptor; state: "restricted"; observedAt: string }
  | { owner: BasisOwnerDescriptor; state: "stale"; observedAt: string }
  | { owner: BasisOwnerDescriptor; state: "corrupt"; observedAt: string }
  | { owner: BasisOwnerDescriptor; state: "unsupported-version"; observedAt: string }
  | { owner: BasisOwnerDescriptor; state: "unavailable"; observedAt: string };
...
export interface BasisContribution { ref: BasisContributionRef; answer: ThreadAnswerRef; role: BasisContributionRole; context: BasisContextProjection; relationships?: readonly BasisContextRelationship[]; }
...
export interface BasisContextRelationship { contract: { authority: string; schemaVersion: string; kind: "basis-context-relationship" }; kind: BasisContextRelationshipKind; from: BasisContributionRef; to: BasisContributionRef; }
...
export interface AnswerAssessmentProjection {
  ...
  /** Only an explicit Surface policy-evaluation outcome may populate this. */ policy: { id: string; outcome: "satisfied" | "not-satisfied" } | null;
...
export interface BasisCompositionInput { version: typeof SURFACE_BASIS_VERSION; answer: OwnerRead<ThreadAnswerObservation>; assessment: OwnerRead<AnswerAssessmentProjection, SurfaceAnswerAssessmentRef>; contributions: readonly OwnerRead<readonly BasisContribution[]>[]; }
...
export interface BasisProjection {
  version: typeof SURFACE_BASIS_VERSION; answer: OwnerRead<ThreadAnswerObservation>; standing: BasisStanding; unresolvedReason: BasisUnresolvedReason | null; assessment: OwnerRead<AnswerAssessmentProjection, SurfaceAnswerAssessmentRef>;
