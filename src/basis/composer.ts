...
export function composeBasisProjectionV2(input: BasisCompositionInputV2): BasisProjectionV2 {
  ...
  const all: BasisContributionV2[] = [];
  for (const read of parsed.value.contributions) if (read.state === "available") all.push(...read.value);
  for (const contribution of all) regions[regionForV2(contribution.role)].push({ ref: contribution.ref, role: contribution.role, context: contribution.context, gaps: [...(contribution.gaps ?? [])] });
  ...
  const result: BasisProjectionV2 = { ...base, version: SURFACE_BASIS_V2_VERSION, regions };
...
function assessmentRelationships(input: BasisCompositionInput): BasisRelationship[] { if (input.assessment.state !== "available" || !input.assessment.value.found || !input.assessment.value.claim) return []; const assessment = input.assessment.value; const claim = assessment.claim!.id; return [...assessment.evidence.cited.map((item) => ({ kind: "cites" as const, from: claim, to: `evidence:${item.id}`, source: "surface-assessment" as const, gaps: [] })), ...assessment.evidence.entails.filter((item) => item.result !== "failed").map((item) => ({ kind: "supports" as const, from: `evidence:${item.id}`, to: claim, source: "surface-assessment" as const, gaps: [] })), ...assessment.evidence.counterevidence.map((item) => ({ kind: "counterevidence" as const, from: `evidence:${item.id}`, to: claim, source: "surface-assessment" as const, gaps: [] })), ...assessment.derivation.directInputs.map((item) => ({ kind: "derived-from" as const, from: claim, to: `claim:${item.claimId}`, source: "surface-assessment" as const, gaps: [] }))]; }
