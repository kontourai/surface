...
(() => {
  ...
  class SurfaceTrustPanel extends HTMLElement {
    ...
    #renderBasis(model: BasisPanelViewModel): void {
      ...
      const gaps = model.gaps.map((gap) => `<li part="gap-row">${escapeHtml(gap.code)} — ${escapeHtml(gap.message)}${gap.metadata ? ` Weak edge${gap.metadata.weakEdges.length === 1 ? "" : "s"}: ${gap.metadata.weakEdges.map((edge) => `${escapeHtml(edge.claimId)} → ${escapeHtml(edge.inputClaimId)}`).join(", ")}.` : ""}</li>`).join("");
      ...
      const assessment = model.assessment ? `<details class="basis-section" part="assessment" ${disclosureOpen(model.disclosures.assessment)}><summary>Assessment</summary><dl><dt>Claim status</dt><dd>${escapeHtml(model.assessment.claimStatus ?? "not available")}</dd><dt>Freshness</dt><dd>${escapeHtml(model.assessment.freshness ?? "not stated")}</dd></dl>${model.assessment.policy ? `<p>Policy: ${escapeHtml(model.assessment.policy.id)} · ${escapeHtml(model.assessment.policy.outcome)} · evaluated ${escapeHtml(model.assessment.policy.evaluatedAt)}${model.assessment.policy.reasons.length ? ` · ${model.assessment.policy.reasons.map(escapeHtml).join(", ")}` : ""}</p>` : ""}${model.assessment.evidence.map((partition) => `<div><h3>${escapeHtml(partition.label)}</h3><ul class="basis-list" part="evidence">${partition.items.map((item) => `<li part="evidence-item">${escapeHtml(item.label)} — ${escapeHtml(item.source)}${item.locator ? ` · ${escapeHtml(item.locator)}` : ""} · ${escapeHtml(item.observedAt)} · ${escapeHtml(item.supportStrength ?? "support not declared")} · ${escapeHtml(item.result)}${item.blocksClaim ? " · blocks claim" : ""}</li>`).join("") || "<li>None recorded.</li>"}</ul></div>`).join("")}${model.assessment.derivation.available ? `<div><h3>Direct derivation</h3><ul class="basis-list" part="derivation">${model.assessment.derivation.directInputs.map((item) => `<li>${escapeHtml(item.claimId)} — ${escapeHtml(item.source)}${item.status ? ` · ${escapeHtml(item.status)}` : ""}${item.method ? ` · ${escapeHtml(item.method)}` : ""}${item.supportStrength ? ` · ${escapeHtml(item.supportStrength)}` : ""}${item.rationale ? ` · ${escapeHtml(item.rationale)}` : ""}</li>`).join("") || "<li>None recorded.</li>"}</ul></div>` : "<p>Direct derivation is unavailable.</p>"}</details>` : `<section class="basis-section" part="assessment"><h3>Assessment</h3><p>No Surface assessment is available.</p></section>`;
