...
(() => {
  ...
  const PANEL_CSS = `
    ...
    .basis-section summary { cursor: pointer; font-weight: 700; min-height: 32px; }
  ...
  class SurfaceTrustPanel extends HTMLElement {
    ...
    #renderBasis(model: BasisPanelViewModel): void {
      ...
      const assessment = model.assessment ? `<section class="basis-section" part="assessment"><h3>Assessment</h3>${model.assessment.policy ? `<p>${escapeHtml(model.assessment.policy)}</p>` : ""}${model.assessment.evidence.map((partition) => `<div><h3>${escapeHtml(partition.label)}</h3><ul class="basis-list" part="evidence">${partition.items.map((item) => `<li part="evidence-item">${escapeHtml(item.label)} — ${escapeHtml(item.sourceRef)} · ${escapeHtml(item.observedAt)}</li>`).join("") || "<li>None recorded.</li>"}</ul></div>`).join("")}</section>` : `<section class="basis-section" part="assessment"><h3>Assessment</h3><p>No Surface assessment is available.</p></section>`;
      const context = `<details class="basis-section" part="context"><summary>Context — ${escapeHtml(model.contextNotice)}</summary>${model.contextGroups.map((group) => `<section><h3>${escapeHtml(group.label)}</h3><ul class="basis-list">${group.items.map((item) => `<li>${escapeHtml(item.context.kind)}${item.gaps.length ? ` — ${item.gaps.map((gap) => escapeHtml(gap.message)).join("; ")}` : ""}</li>`).join("") || "<li>None recorded.</li>"}</ul></section>`).join("")}</details>`;
      const relationships = `<details class="basis-section" part="relationships"><summary>Relationships</summary><ul class="basis-list">${model.relationships.map((item) => `<li>${escapeHtml(item.prose)}</li>`).join("") || "<li>No Surface assessment relationships recorded.</li>"}</ul></details>`;
      ...
      this.#renderShell(`<header class="panel-header" part="header"><h2 class="panel-title" part="title">Basis</h2></header><p class="basis-standing" part="standing" data-kind="${escapeHtml(model.standing.tone)}">${escapeHtml(model.standing.label)} — ${escapeHtml(model.standing.description)}</p><section class="basis-section" part="gaps"><h3>Gaps (${model.gaps.length})</h3><ul class="basis-list">${gaps || "<li>None recorded.</li>"}</ul></section>${assessment}${context}${relationships}${technical}<p class="footnote" part="footer">Context does not establish support. Inspect Surface assessment evidence before relying on an answer.</p>`);
    }
  }
