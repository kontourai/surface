...
(() => {
  ...
  class SurfaceTrustPanel extends HTMLElement {
    ...
    #renderBasis(model: BasisPanelViewModel): void {
      ...
      const assessment = model.assessment ? `<details class="basis-section" part="assessment" ${disclosureOpen(model.disclosures.assessment)}><summary>Assessment</summary>${model.assessment.policy ? `<p>${escapeHtml(model.assessment.policy)}</p>` : ""}${model.assessment.evidence.map((partition) => `<div><h3>${escapeHtml(partition.label)}</h3><ul class="basis-list" part="evidence">${partition.items.map((item) => `<li part="evidence-item">${escapeHtml(item.label)} — ${escapeHtml(item.source)} · ${escapeHtml(item.observedAt)}</li>`).join("") || "<li>None recorded.</li>"}</ul></div>`).join("")}</details>` : `<section class="basis-section" part="assessment"><h3>Assessment</h3><p>No Surface assessment is available.</p></section>`;
      ...
      const relationships = `<details class="basis-section" part="relationships" ${disclosureOpen(model.disclosures.relationships)}><summary>Relationships</summary><ul class="basis-list">${model.relationships.map((item) => `<li><strong>${escapeHtml(item.label)}</strong> — ${escapeHtml(item.prose)} ${escapeHtml(item.from.value)} → ${escapeHtml(item.to.value)}</li>`).join("") || "<li>No Surface assessment relationships recorded.</li>"}</ul></details>`;
