...
Start here when you need the architecture in Surface vocabulary first. For the core vocabulary, see [Concepts](../product/concepts.md). For current JSON contracts, see [Schemas](../reference/schemas.md) and [Open Trust Format and Claim Package Shape](../specs/open-trust-format.md). For product-layer boundaries, see [Architecture](index.md), [Surface Foundation Boundary](surface-foundation.md). For how derived trust is bounded and how impact/drilldown are computed, see [Derivation & Counterfactual Algorithms](derivation-and-counterfactual-algorithms.md).
