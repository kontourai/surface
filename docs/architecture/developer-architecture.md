...
Start here when you need the architecture in Surface vocabulary first. For the core vocabulary, see [Concepts](../concepts.md). For current JSON contracts, see [Schemas](../schemas.md) and [Open Trust Format and Claim Package Shape](../specs/open-trust-format.md). For product-layer boundaries, see [Architecture](../architecture.md), [Surface Foundation Boundary](surface-foundation.md), and [What builds on Surface](../built-on-surface.md). For future Resource Contract direction, see [Resource Contract Audit](../resource-contract-audit.md).
...
1. [Concepts](../concepts.md) for claim, evidence, Authority Trace, Trust Snapshot, Transparency Gap, and Surface Console language.
2. [Open Trust Format](../specs/open-trust-format.md) for the portable claim package commitment.
3. [Schemas](../schemas.md) for current `TrustInput`, `TrustReport`, and record shapes.
4. [Surface Foundation Boundary](surface-foundation.md) for what belongs in Surface versus product layers.
5. [Resource Contract Audit](../resource-contract-audit.md) only when planning future Resource Contract migration work.
