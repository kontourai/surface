...
| Projections | `src/analytics.ts`, `src/linked.ts`, `src/derivation-drilldown.ts`, `src/answer-card-projection.ts` | Public | Keep public | These are useful Builder, Verifier, and Agent Mode read models over reports. `answer-card-projection.ts` stays a compact report-only adapter; it must not become a second status, freshness, or support derivation seam. |
