...
| Projections | `src/analytics.ts`, `src/linked.ts`, `src/derivation-drilldown.ts`, `src/answer-card-projection.ts`, `src/basis/` | Public | Keep public | These are useful Builder, Verifier, and Agent Mode read models over reports. `answer-card-projection.ts` stays a compact report-only adapter; `basis/` is a separately exported headless boundary for a Thread answer's existing Surface assessment plus inert owner context. Neither may become a second status, freshness, or support derivation seam. |
...
| Surface Console runtime | `src/console/server.ts`, `src/console/runtime.ts`, `src/console/projection.ts`, `src/console/claim-detail-projection.ts`, `src/console/types.ts` | Partly public through `startConsoleServer` and config types | Keep public entry minimal | `startConsoleServer` is currently exported. `server.ts` holds HTTP transport only; `SurfaceConsoleRuntime` (`runtime.ts`) owns read-model loading, run catalog, projection creation, HTML assembly, and Claim Package authoring, and `claim-detail-projection.ts` precomputes the per-claim detail model. Routes and runtime internals should stay Surface-owned until a real external embedding use case requires a deeper Console interface. |
