...
| `surface_waiver_validity` | `claimId?`, `input?`, `adapter?` | Per-claim [accepted-gap waiver validity](waiver-validity.md) verdicts (bare-assumed, complete-waiver, incomplete-waiver, stale-or-revoked-waiver, command-backed-waiver-rejection); with `claimId`, just that claim's verdict, otherwise every claim in the report |
